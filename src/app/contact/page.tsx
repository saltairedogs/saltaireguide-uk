...
//
// NOTE: The form posts to /api/contact (not implemented yet).
// You can wire that endpoint to your email provider later. We also include a mailto fallback.
...
export const metadata: Metadata = {
  title: 'Contact Saltaire Guide — tips, corrections, media & partnerships',
  description:
    'Say hello to Saltaire Guide. Send editorial tips, corrections, accessibility feedback, media/permissions and partnership enquiries. We usually respond within 1–2 working days.',
  alternates: { canonical: `${site.url}/contact` },
  openGraph: {
    title: 'Contact Saltaire Guide',
    description:
      'Editorial tips, corrections, accessibility feedback, media/permissions, partnerships. Local, practical and responsive.',
...
const CATEGORIES = [
  ...
  { value: 'media', label: 'Media & permissions (photo use, quotes, interviews)' },
  { value: 'partnership', label: 'Partnerships (listings, collabs, community projects)' },
  { value: 'accessibility', label: 'Accessibility feedback (step-free routes, facilities)' },
...
const FAQS: Array<{ q: string; a: string }> = [
  ...
  },
  {
    ...
    a: 'Shipley & Saltaire. We walk the routes we recommend and update guides regularly.',
  },
]
  ...
  return (
    <div className="field">
      <label htmlFor={id} className="label">
        {label} {required ? <span className="text-gray-500" aria-hidden="true">*</span> : null}
      </label>
      {children}
  ...
  return (
    <header className="bg-gradient-to-b from-white to-gray-50 border-b border-gray-200/70">
      <div className="container mx-auto grid max-w-7xl items-center gap-8 px-4 py-10 md:grid-cols-2 md:py-14">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight md:text-5xl">Contact Saltaire Guide</h1>
          <p className="mt-4 max-w-prose text-lg text-gray-700">
            Send tips, corrections, accessibility notes, media/permissions and partnership ideas. We keep responses
            practical and local, and we never sell your data.
          </p>
        ...
        <div className="card card-muted">
          <div className="card-body">
            <div className="text-lg font-semibold">Prefer email?</div>
            <p className="mt-2 text-gray-700">
              Email us directly at{' '}
              <a className="underline underline-offset-4" href={`mailto:${site.email}`}>
                {site.email}
              </a>
              . For time-sensitive updates (closures, diversions), add <strong>“URGENT”</strong> to the subject.
            </p>
            <p className="mt-2 text-sm text-gray-600">
              You can also reach us via <Link className="underline underline-offset-4" href="/about">About</Link> or see{' '}
              <Link className="underline underline-offset-4" href="/legal/editorial-policy">
                Editorial policy
              </Link>{' '}
              and <Link className="underline underline-offset-4" href="/legal/corrections">Corrections</Link>.
            </p>
          </div>
        </div>
      </div>
...
function Reasons() {
  const items = [
    { title: 'Editorial tip', note: 'New café opening, route changes, closures, step-free improvements.' },
    { title: 'Correction', note: 'Prices, opening hours, surfaces, diversions, accessibility facilities.' },
    { title: 'Media & permissions', note: 'Quote requests, interviews, photo licensing and attribution.' },
    { title: 'Partnerships', note: 'Community projects, data sharing, local events and maps.' },
    { title: 'Accessibility feedback', note: 'Wheelchair/pram routes, toilets, gradients, cobbles and signage.' },
  ]
  return (
    <section id="reasons" aria-labelledby="reasons-title" className="container mx-auto max-w-7xl px-4 py-10">
      <SectionHeading id="reasons-title">Why people get in touch</SectionHeading>
      <p className="mt-2 max-w-prose text-gray-700">
        We aim to keep guides both welcoming and accurate. Your local knowledge helps thousands of visitors every month.
      </p>
...
function ContactForm() {
  return (
    <section id="form" aria-labelledby="form-title" className="container mx-auto max-w-7xl px-4 py-10">
      <SectionHeading id="form-title">Send a message</SectionHeading>
      <p className="mt-2 max-w-prose text-gray-700">
        Use the form below or email <a className="underline underline-offset-4" href={`mailto:${site.email}`}>{site.email}</a>.
      ...
      <FormBridge formName="Contact: general" className="mt-6 grid gap-5 rounded-2xl border border-gray-200 bg-white p-6 md:p-8" noValidate>
        ...
        <div className="grid gap-5 md:grid-cols-2">
          ...
          <Field id="email" label="Email" required hint="We’ll only use this to reply to your message.">
            <input
              id="email"
              name="email"
              type="email"
        ...
        <Field id="category" label="What’s this about?" required>
          <select id="category" name="category" className="select" required aria-required="true" defaultValue="">
        ...
        <Field id="subject" label="Subject" required>
          <input id="subject" name="subject" type="text" className="input" placeholder="Short summary" required />
        </Field>

        <Field id="message" label="Message" required hint="Include relevant dates, places and links.">
          <textarea
            id="message"
            name="message"
            className="textarea"
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <Field id="consent" label="Consent">
            <label className="flex items-center gap-2 text-sm text-gray-700">
              <input id="consent" name="consent" type="checkbox" className="h-4 w-4" required aria-required="true" />
          ...
          <Field id="reply" label="Preferred reply">
            <select id="reply" name="reply" className="select" defaultValue="email">
              ...
              <option value="no-reply">No reply needed (just FYI)</option>
            </select>
          </Field>
        </div>
      ...
      <div className="mt-4 smallprint">
        ...
        <Link className="underline underline-offset-4" href="/events">What’s On</Link>.
      </div>
    </section>
  )
}
...
function ExtraInfo() {
  return (
    <section id="extra" aria-labelledby="extra-title" className="container mx-auto max-w-7xl px-4 py-10">
      ...
      <div className="mt-4 grid gap-6 md:grid-cols-2">
        <div className="callout">
          ...
          <p className="mt-2 text-gray-700">
            If your message is a correction (prices, hours, paths, access), please paste a link or photo of signage if
            you have one. We’ll verify and update quickly—especially for accessibility details.
          </p>
          <p className="mt-2 text-sm text-gray-700">
            See our <Link className="underline underline-offset-4" href="/legal/corrections">Corrections policy</Link>.
          </p>
        </div>
        <div className="callout callout-success">
          <h3 className="text-lg font-semibold">Permissions & media</h3>
          <p className="mt-2 text-gray-700">
            For image use and interviews, tell us the outlet, deadline and the specific images/text you’d like to use.
            We’ll confirm permissions and the attribution required.
          </p>
          <p className="mt-2 text-sm text-gray-700">
      ...
      <div className="container mx-auto max-w-7xl px-4 py-10 md:py-14">
        <div className="grid items-center gap-8 md:grid-cols-2">
          <div>
            ...
            <p className="mt-2 max-w-prose text-gray-700">
              We’re independent and local. No sponsored posts, no fluff—just practical guides with original photos and
              accessibility notes.
            </p>
          ...
          <div className="card card-muted">
            <div className="card-body">
              ...
              <p className="mt-2 text-gray-700">
                Email <a className="underline underline-offset-4" href={`mailto:${site.email}`}>{site.email}</a> or
                message us via the social links on the <Link className="underline underline-offset-4" href="/about">About page</Link>.
              </p>
              <p className="mt-2 text-sm text-gray-700">We’ll never share your details with third parties.</p>
            </div>
          </div>
        </div>
      </div>
...
function JsonLd() {
  ...
  const page = {
    ...
    url: `${base}/contact`,
    description:
      'Send tips, corrections, accessibility feedback, media/permissions and partnership enquiries to Saltaire Guide.',
    speakable: {
